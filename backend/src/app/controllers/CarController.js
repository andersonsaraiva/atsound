...
class CarController {
  ...
  async create(req, res) {
    try {
      const { model, color, year, license_plate } = req.body;
      const { customer_id } = req.params;
      ...
      await Car.create({ customer_id, model, color, year, license_plate });
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (car) await car.update({ customer_id, description, price });
